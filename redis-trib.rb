...
ALLOWED_OPTIONS={
    "create" => {"replicas" => true, "password" => ""},
    "check" => {"password" => ""},
    "info" => {"password" => ""},
    "add-node" => {"slave" => false, "master-id" => true, "password" => ""},
    "del-node" => {"password" => ""},
    "set-timeout" => {"password" => ""},
    "call" => {"password" => ""},
    "import" => {"from" => :required, "copy" => false, "replace" => false, "password" => ""},
    "reshard" => {"from" => true, "to" => true, "slots" => true, "yes" => false, "timeout" => true, "pipeline" => true, "password" => ""},
    "rebalance" => {"weight" => [], "auto-weights" => false, "use-empty-masters" => false, "timeout" => true, "simulate" => false, "pipeline" => true, "threshold" => true, "password" => ""},
    "fix" => {"timeout" => MigrateDefaultTimeout, "password" => ""},
}

def show_help
    puts "Usage: redis-trib <command> <options> <arguments ...>\ndefult support host:port:password, but --password Recommended\n\n"
    COMMANDS.each{|k,v|
        puts "  #{k.ljust(15)} #{v[2]}"
